...
let div_beratStandard = document.getElementById("berat_standard"); //untuk div berat standard (KGM)
...
let fakturPjkBiasa = document.getElementById("faktur_pjkBiasa");
let fakturPjkSederhana = document.getElementById("faktur_pjkSederhana");
...
syaratBayar.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        fakturPjkBiasa.focus();
    }
});

fakturPjkBiasa.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        keterangan.focus();
    }
...
fakturPjkSederhana.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        keterangan.focus();
    }
...
tglPesan.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        jenisSp.focus();
    }
...
tglPo.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        noPi.focus();
    }
...
jenisSp.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        list_customer.focus();
    }
...
listSales.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        mataUang.focus();
    }
...
jenisBayar.addEventListener("keypress", function (event) {
    if (event.key === "Enter") {
        event.preventDefault();
        syaratBayar.focus();
    }
    ...
    if (event.key === "Enter") {
        ...
        fetch("/editSP/" + no_spText.value)
            ...
            .then((data) => {
                ...
                noPo.readOnly = false;
                noPi.readOnly = false;
                tglPo.readOnly = false;
                listSales.disabled = false;
                mataUang.readOnly = false;
                jenisBayar.disabled = false;
                syaratBayar.readOnly = false;
                keterangan.readOnly = false;
                fakturPjkBiasa.disabled = false;
                fakturPjkSederhana.disabled = false;

                // console.log(data[0][0].Tgl_Pesan.substr(0, 10));
                tglPesan.value = data[0][0].Tgl_Pesan.substr(0, 10);
                const optionJenisSp = jenisSp.options;
                ...
                tglPo.value = data[0][0].Tgl_PO.substr(0, 10);
                noPi.value = data[0][0].NO_PI;
                const optionSales = listSales.options;
                ...
                const optionJenisBayar = jenisBayar.options;
                ...
                if (JnsFakturPjk == 0) {
                    fakturPjkBiasa.checked = true;
                } else {
                    fakturPjkSederhana.checked = true;
                }
...
function funcClearHeaderPesanan() {
    tglPesan.valueAsDate = new Date();
    jenisSp.selectedIndex = 0;
    no_spText.value = "";
    listSales.selectedIndex = 0;
    list_customer.selectedIndex = 0;
    noPo.value = "";
    tglPo.valueAsDate = new Date();
    noPi.value = "";
    mataUang.value = "";
    jenisBayar.selectedIndex = 0;
    syaratBayar = "";
    fakturPjkBiasa.checked = true;
    keterangan.value = "";
}
...
edit_button.addEventListener("click", function (event) {
    ...
    if (proses == 0) {
        ...
        syaratBayar.readOnly = true;
        keterangan.readOnly = true;
        fakturPjkBiasa.disabled = true;
        fakturPjkSederhana.disabled = true;
    } else {
