jQuery(function ($) {
        ...
        if (!(masuk.checked || keluar.checked)) {
            ...
            `,
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (objekNama.value !== "") {
                            $("#tableData").show();
                            var selectedRow = table.$("tr.selected").length;
                            if (selectedRow) {
                                return; // saat ada row yang dipilih tidak perlu refresh table supaya bisa pilih data barang yang baru
                            }
    ...
    primer2.addEventListener("keypress", function (event) {
        if (event.key === "Enter") {
            if (
                numeral(primer2.value).value() >
                    numeral(primer.value).value() &&
                keluar.checked
            ) {
                Swal.fire({
                    icon: "error",
            ...
            if (
                ...
                Swal.fire({
                    ...
                    text:
                        ...
                        numeral(numeral(sekunder.value).value()).format(
                            "0,0.00"
                        ),
                    returnFocus: false,
                }).then(() => {
    ...
    function getSaldoPIB(kodeType, PIB) {
        $.ajax({
            url: "MhnMasukKeluar/getSaldoPIB",
            type: "GET",
            data: {
                _token: csrfToken,
                kodeType: kodeType,
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response && response.length > 0) {
                    ...
                    no_sekunder.textContent = response[0].Satuan_Sekunder.trim(); // prettier-ignore
                    no_tritier.textContent = response[0].Satuan_Tritier.trim();
                    primerValue = response[0].Satuan_Primer.trim();
                    sekunderValue = response[0].Satuan_Sekunder.trim();
                    tritierValue = response[0].Satuan_Tritier.trim();
                }
            },
            error: function (xhr, status, error) {
                console.error("AJAX Error:", error);
    ...
    $("#tableData tbody").on("click", "tr", function () {
        ...
        tanggal.value = moment(data[7], "MM/DD/YYYY").format("YYYY-MM-DD");

        user.value = data[8];
        noPIB.value = data[9];

        divisiId.value = data[10];
        objekId.value = data[11];
        kelutId.value = data[13];
        kelutNama.value = decodeHtmlEntities(data[14]);
        kelompokId.value = data[15];
        kelompokNama.value = decodeHtmlEntities(data[16]);
        subkelId.value = data[17];
        subkelNama.value = decodeHtmlEntities(data[18]);

        no_primer.textContent = data[19] || "";
        no_sekunder.textContent = data[20] || "";
        no_tritier.textContent = data[21] || "";
        kodeBarang.value = data[22];

        getType(kodeType.value);
        getSaldoPIB(kodeType.value, noPIB.value);
        if (a === 1 || a === 2) {
            primer2.select();
        } else {
            btn_proses.focus();
        ...
        if (a === 1 || a === 2) {
            ...
            if (
                ...
                Swal.fire({
                    ...
                    text: "Jumlah barang harus diisi dan sesuai saldo yang ada",
                    returnFocus: false,
                }).then(() => {
                    tritier2.focus();
                });
